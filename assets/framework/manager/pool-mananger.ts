...
export default class PoolManager {
     ...
     */
    addPoolToMap(poolname: string | Pool_Name, perfabs: Prefab | string, poolSize: number = 10, poolScript?: string |  __private._extensions_ccpool_node_pool__Constructor<__private._extensions_ccpool_node_pool__IPoolHandlerComponent> ): MPromise<boolean> {
...
//      * 该组件类型会调用 unuse/reuse方法。可以参考：https://blog.csdn.net/qq_45021180/article/details/104484744 加深理解
//      */
//     addPoolToMap(poolname: string | Pool_Name, perfabs: Prefab | string, poolSize: number = 10, poolScript?: string | { prototype: Component; }): MPromise<boolean> {
//         return new MPromise((reslove, reject) => {
//             let tempPool = new NodePool(poolScript);
//             if (typeof perfabs === 'string') {
//                 ResourceManager.getInstance().LoadResPerfab(perfabs).then(
